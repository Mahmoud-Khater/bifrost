...
export function LogsHeaderView({
	...
	return (
		<div className="flex grow items-center justify-between space-x-2">
			<Button variant="outline" size="sm" className="h-7.5 disabled:opacity-100" onClick={() => fetchLogs()} disabled={loading}>
			...
			<DateTimePickerWithRange
				...
				onDateTimeUpdate={(p) => {
					setStartTime(p.from);
					setEndTime(p.to);
					// Custom absolute range: clear the period and update filters
				...
				preDefinedPeriods={TIME_PERIODS as unknown as { label: string; value: string }[]}
