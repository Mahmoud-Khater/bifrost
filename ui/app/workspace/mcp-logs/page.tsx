...
export default function MCPLogsPage() {
	...
	const { data: statsData, isFetching: statsIsFetching } = useGetMCPLogsStatsQuery({ filters }, { refetchOnMountOrArgChange: true });
	...
	// Period + no-polling: update URL timestamps on focus; period + polling: interval updates them
	...
	const handlePollToggle = useCallback(
		(enabled: boolean) => {
			...
			if (enabled) refetchLogs();
		},
		[setUrlState, refetchLogs],
	);
	...
	const statCards = useMemo(
		() => [
			...
			{
				title: "Total Cost",
				value: <NumberFlow value={statsData?.total_cost ?? 0} format={{ ...COMPACT_NUMBER_FORMAT, style: "currency", currency: "USD" }} />,
